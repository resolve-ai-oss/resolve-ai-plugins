...
- Per-investigation one-liner: `title`, `status`, `stop_reason`, `run_type`, `use_case`, `team`, primary alert summary if present, `created_at`, and the canvas URL (`<your-resolve-host>/chat/<investigation_id>`).
- When the list is long, group by `status`, `team`, or `run_type` and offer to narrow.
