...
- **Command tag** (prefix `▷`): name the slash command for each beat's capability. In **auto** mode it's just the label, so the user learns it — `▷ Run it yourself: /skill:overview`. In **manual** mode it's the _full runnable command with composed args_, and you stop and wait for the user to enter it — e.g. `▷ Type this: /skill:ask Follow-up: of the error spikes in svc-analysis, which one is most worth acting on first?`. Mapping (args from each skill's own `argument-hint`): surface → `/skill:overview` (focused lists `/skill:alerts`, `/skill:investigations`, `/skill:chats`); resume / ask / thread / parallel → `/skill:ask <message>`; redirect a live investigation → `/skill:steer <message>`; drill or start an RCA → `/skill:investigate <url-or-id | problem>`; apply a fix → `/skill:apply-fix`.
...
**The big picture** — `/skill:overview` shows investigations, recent alerts, and in-flight chats at a glance. (`/skill:alerts` for just the firing feed that auto-triggers investigations; `/skill:help-resolve` to get oriented.)

**Investigations** — structured root-cause workspaces (theories, cited evidence, mitigations, tied to the triggering alert):

- `/skill:investigate` — open an existing RCA or start a new one
- `/skill:investigations` — list recent investigations
- `/skill:ask` — ask a question or open a thread on an investigation
- `/skill:steer` — redirect a running investigation with a new finding
- `/skill:apply-fix` — turn its findings into a code change, right here

**Chats** — standalone conversations with Resolve — ask anything about your environment:

- `/skill:ask` — start or continue a standalone chat
- `/skill:chats` — list recent chats
...
`▷ Run it yourself: /skill:apply-fix`
...
One line recapping the loop, then hand over the controls — the skills they can run themselves: `/skill:overview`, `/skill:ask`, `/skill:investigate`, `/skill:alerts` / `/skill:investigations` / `/skill:chats`, `/skill:steer`, `/skill:apply-fix`.
